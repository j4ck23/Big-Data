...
dfVMM = dfNum.withColumn("Status",col("Status").cast(IntegerType()))
...
output = assembler.transform(dfVMM)
...
#The linear suport vector, fits the training data
Vec = LinearSVC(maxIter=100, regParam=0.1,labelCol='Status').fit(train)

#applys the test data to the trained model
pre = Vec.transform(test)
...
plt.title('Support Vector, Normal = 1, Abnormal = 0', fontsize = 10) # title with fontsize 10
plt.show()